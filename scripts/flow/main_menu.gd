...
func _ready() -> void:
	%LanguagePicker.select(0 if TranslationServer.get_locale().begins_with("zh") else 1)
	%LanguagePicker.item_selected.connect(func(index: int):
		get_node("/root/Localization").set_language("zh_CN" if index == 0 else "en"))
	get_node("/root/Localization").language_changed.connect(_refresh_language)
...
func _refresh_room() -> void:
	...
	if session.phase == session.Phase.LOBBY and current_page != 3:
		show_page(3)
	%LocalName.text = tr("你（房主） · 选择势力") if session.is_local_host() else tr("你（加入者） · 选择势力")
	%RoomAddress.text = tr("主机地址：") + session.room_addresses() if session.is_local_host() else tr("已连接主机 · TCP %d") % session.room_port
	...
	%RoomCount.text = tr("%d / 2 位玩家") % players.size()
	%GuestStatus.text = tr("等待另一位玩家加入…")
	for player_id: int in players:
		if player_id != session.local_player_id:
			%GuestStatus.text = "%s\n%s" % [tr(players[player_id].name), tr(session.faction_name(int(players[player_id].faction_id)))]
	var reason: String = tr(session.start_block_reason())
	%StartButton.disabled = not reason.is_empty()
	%StartButton.text = (tr("单人启动游戏") if players.size() == 1 else tr("开始对局")) if session.is_local_host() else tr("等待房主开始")
	%LobbyStatus.text = reason if not reason.is_empty() else (tr("可以等待对手，也可以直接单人进入地图。") if players.size() == 1 else tr("双方势力不同，可以开始对局。"))
...
func _request_join() -> void:
	...
	if result == ERR_INVALID_PARAMETER:
		_join_error_key = "请输入主机地址。" if %AddressInput.text.strip_edges().is_empty() else "请输入有效 IPv4 地址，可附加 :端口。"
		%JoinStatus.text = tr(_join_error_key)
	else:
		_join_error_key = ""
		%JoinStatus.text = session.status_message
	%ConnectButton.disabled = session.connecting
	%ConnectButton.text = tr("正在连接…") if session.connecting else tr("连接房间")
...
func _start() -> void:
	...
	if result != OK:
		%LobbyStatus.text = tr(session.start_block_reason()) if result == ERR_UNAUTHORIZED else tr("无法载入对局：") + error_string(result)
...
func _refresh_language() -> void:
	...
	%ConnectButton.text = tr("正在连接…") if session.connecting else tr("连接房间")
	_refresh_room()
	if not session.status_message.is_empty():
		_network_status(session.status_message)
	elif not _join_error_key.is_empty():
		%JoinStatus.text = tr(_join_error_key)
